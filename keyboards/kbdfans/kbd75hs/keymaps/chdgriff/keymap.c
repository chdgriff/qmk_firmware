#include QMK_KEYBOARD_H
#include "process_magic.h"

#define _Base_Layer             0
#define _Blank_Layer            1
#define _Function_Layer         2
#define _Blank_Function_Layer   3
#define HSV_MY_DEFAULT          160,    242,    255 // H S V
#define HSV_MY_YELLOW           22,     255,    220
#define IDLE_TIMEOUT_MS         120000  // Idle timeout in milliseconds.

enum custom_keycodes {
    LOCK = SAFE_RANGE,
    UNLOCK,
};

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_Base_Layer] = LAYOUT_75_ansi(
        KC_ESC,   KC_F1,   KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8,   KC_F9,     KC_F10,    KC_F11,  KC_F12, KC_HOME,  KC_PSCR, KC_END,
        KC_GRV,   KC_1,    KC_2,  KC_3,  KC_4,  KC_5,  KC_6,  KC_7,  KC_8,    KC_9,      KC_0,      KC_MINS, KC_EQL, KC_BSPC,          KC_INS,
        KC_TAB,     KC_Q,    KC_W,  KC_E,  KC_R,  KC_T,  KC_Y,  KC_U,  KC_I,     KC_O,     KC_P,       KC_LBRC, KC_RBRC, KC_BSLS,      KC_DEL,
        KC_CAPS,      KC_A,    KC_S,  KC_D,  KC_F,  KC_G,  KC_H,  KC_J,  KC_K,     KC_L,     KC_SCLN,     KC_QUOT, KC_ENT,             KC_PGUP,
        KC_LSFT,        KC_Z,    KC_X,  KC_C,  KC_V,  KC_B,  KC_N,  KC_M,  KC_COMM,  KC_DOT,    KC_SLSH,      KC_RSFT,        KC_UP,   KC_PGDN,
        KC_LCTL, KC_LALT, KC_LGUI, KC_SPC,                                    MO(_Function_Layer), KC_RALT,  KC_RCTL, KC_LEFT, KC_DOWN, KC_RGHT
    ),
    [_Blank_Layer] = LAYOUT_75_ansi(
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, MO(_Blank_Function_Layer), KC_NO, KC_NO, KC_NO, KC_NO, KC_NO
    ),
    [_Function_Layer] = LAYOUT_75_ansi(
        LOCK, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, KC_MUTE, KC_MPRV, KC_MPLY, KC_MNXT,
        DB_TOGG,  _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, QK_RBT,                                QK_BOOT,
        _______,              _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                              _______,
        _______,                  _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                                   _______,
        _______,                      _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                   _______,             _______,
        CG_TOGG, _______, GU_TOGG, _______,                                                _______, _______, _______, _______,             _______,             _______
    ),
    [_Blank_Function_Layer] = LAYOUT_75_ansi(
        UNLOCK, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO,
        KC_NO, KC_NO, KC_NO, KC_NO, _______, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO
    )
};

const rgblight_segment_t PROGMEM capslock_light_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 5, HSV_MY_YELLOW}       // Light 5 LEDs, starting with LED 0
);

const rgblight_segment_t PROGMEM function_light_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {15, 2, HSV_WHITE}       // Light 2 LEDs, starting with LED 14
);

const rgblight_segment_t* const PROGMEM my_rgb_layers[] = RGBLIGHT_LAYERS_LIST(
    capslock_light_layer,
    function_light_layer
);

bool led_update_user(led_t led_state) {
    rgblight_set_layer_state(0, led_state.caps_lock);
    return true;
}

layer_state_t layer_state_set_user(layer_state_t state) {
    rgblight_set_layer_state(1, (layer_state_cmp(state, _Function_Layer) || layer_state_cmp(state, _Blank_Function_Layer)));
    return state;
}

static uint32_t idle_sleep_callback(uint32_t trigger_time, void* cb_arg) {
    // If execution reaches here, the keyboard has gone idle.
    rgblight_suspend();
    return 0;
}

static void idle_wake(void) {
    static deferred_token idle_token = INVALID_DEFERRED_TOKEN;
    if (!extend_deferred_exec(idle_token, IDLE_TIMEOUT_MS)) {
        rgblight_wakeup();
        idle_token = defer_exec(IDLE_TIMEOUT_MS, idle_sleep_callback, NULL);
    }
}

// At the end of the firmware's startup process:
void keyboard_post_init_user(void) {
    // Enable the LED layers
    rgblight_layers = my_rgb_layers;
    rgblight_sethsv_noeeprom(HSV_MY_DEFAULT);
    rgblight_mode_noeeprom(RGBLIGHT_DEFAULT_MODE);
    rgblight_enable_noeeprom(); // enables Rgb, without saving settings

    idle_wake();
}

// Before keycode event is processed:
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    idle_wake();

    if (record->event.pressed) {
        switch (keycode) {
            case LOCK:
                layer_on(_Blank_Layer);
                rgblight_sethsv_noeeprom(HSV_RED);
                break;
            case UNLOCK:
                layer_off(_Blank_Layer);
                rgblight_sethsv_noeeprom(HSV_MY_DEFAULT);
                break;
        default:
            break;
        }
    }
    return true;
}
